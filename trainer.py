...
class Trainer:
    ...
    def fit(self, train_loader, val_loader, epochs):
        ...
        print("🛠 DEBUG: Calling _save_model()")

        self._save_model("last")  # <-- Ensure this is running!
       

        plt.plot(train_loss_, label="train_loss")
        plt.plot(val_loss_, label="val_loss")
        plt.legend()
        plt.show()
    ...
    def _train(self, loader):
        ...
        for idx_batch, data_batch in enumerate(pbar := tqdm(loader)):
            ...
            batch_size = x.size(0)
            hidden = self.model.initHidden(batch_size).to(self.device)
            ...
            for i in range(seq_len):
                ...
                # Forward pass with the input and hidden state
                output, hidden = self.model(input, hidden)

                # If this is the last time step, use the output to compute the loss
                if i == seq_len - 1:
                    y_hat = output
